...
export class ReportUIFeatures {
  ...
  initFeatures(lhr) {
    ...
    const disableDarkMode =
      this._dom.isDevTools() || this._opts.disableAutoDarkModeAndFireworks;
    if (
      !disableDarkMode &&
    ...
    const hasMetricError =
      ...
      lhr.categories.performance.auditRefs.some((audit) =>
        Boolean(audit.group === 'metrics' && lhr.audits[audit.id].errorMessage)
      );
    if (hasMetricError) {
      const toggleInputEl = this._dom.find(
        'input.lh-metrics-toggle__input',
        this._dom.rootEl
      );
    ...
    const showTreemapApp =
      this.json.audits['script-treemap-data'] &&
    ...
    if (this._opts.getStandaloneReportHTML) {
      this._dom
        .find('a[data-action="save-html"]', this._dom.rootEl)
        .classList.remove('lh-hidden');
    }
  ...
  addButton(opts) {
    ...
    const metricsEl = this._dom.rootEl.querySelector(
    ...
    if (!buttonsEl)
      buttonsEl = this._dom.createChildOf(metricsEl, 'div', 'lh-buttons');

    const classes = ['lh-button'];
    if (opts.icon) {
  ...
  _setupThirdPartyFilter() {
    ...
    const tables = Array.from(
      this._dom.rootEl.querySelectorAll('table.lh-table')
    );
  ...
  _setupElementScreenshotOverlay(rootEl) {
    const fullPageScreenshot =
      ...
      this.json.audits['full-page-screenshot'].details.type ===
