...
class ReportGenerator {
  ...
  static replaceStrings(source, replacements) {
    ...
    return source.split(firstReplacement.search).map(part => ReportGenerator.replaceStrings(part, nextReplacements)).join(firstReplacement.replacement);
  }
  /**
  ...
  static sanitizeJson(object) {
    return JSON.stringify(object).replace(/</g, '\\u003c') // replaces opening script tags
  ...
  static generateReportHtml(lhr) {
    const sanitizedJson = ReportGenerator.sanitizeJson(lhr); // terser does its own sanitization, but keep this basic replace for when
    ...
    return ReportGenerator.replaceStrings(htmlReportAssets.REPORT_TEMPLATE, [{
      ...
      replacement: sanitizedJson
    }, {
      search: '%%LIGHTHOUSE_JAVASCRIPT%%',
  ...
  static generateFlowReportHtml(flow) {
    ...
    /* eslint-disable max-len */
    {
      ...
      replacement: htmlReportAssets.FLOW_REPORT_JAVASCRIPT
    }, {
  ...
  static generateReportCSV(lhr) {
    ...
    const rowFormatter = row => row.map(value => value.toString()).map(escape); // Possible TODO: tightly couple headers and row values
    ...
    const table = Object.keys(lhr.categories).map(categoryId => {
      ...
      rows.push(rowFormatter([lhr.requestedUrl, lhr.finalUrl, category.title, `${categoryId}-score`, `Overall ${category.title} Category Score`, 'numeric', overallCategoryScore]));
      return rows.concat(category.auditRefs.map(auditRef => {
        const audit = lhr.audits[auditRef.id]; // CSV validator wants all scores to be numeric, use -1 for now

        const numericScore = audit.score === null ? -1 : audit.score;
        return rowFormatter([lhr.requestedUrl, lhr.finalUrl, category.title, audit.id, audit.title, audit.scoreDisplayMode, numericScore]);
      }));
    });
